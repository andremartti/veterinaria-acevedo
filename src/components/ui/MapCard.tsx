...
function StylizedMap() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <rect width="800" height="600" fill="#dcefe6" />
      ...
      <g stroke="#c6bda9" fill="none" strokeWidth="1.2" opacity="0.6">
        <path d="M0 205h800M280 0v600" />
      </g>

      {/* Marcador de la clínica */}
      <g transform="translate(400 300)">
        <circle r="74" fill="#21906f" opacity="0.12" />
        <circle r="46" fill="#21906f" opacity="0.18" />
        <circle r="26" fill="#0f3b30" />
        <g transform="translate(-9 -9) scale(0.75)" fill="#e6f4ed">
...
export function MapCard() {
  ...
  return (
    <div className="relative h-full min-h-[22rem] overflow-hidden rounded-[2rem] border border-forest-900/8 bg-mint-50 shadow-[var(--shadow-lift)] lg:min-h-[32rem]">
      {showEmbed ? (
        <iframe
        ...
        <>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-end gap-4 bg-gradient-to-t from-forest-950/20 via-transparent to-transparent px-6 pb-7 text-center sm:pb-9">
            <div className="hidden rounded-2xl border border-forest-900/8 bg-white/95 px-5 py-4 shadow-[var(--shadow-lift)] sm:block">
              <p className="font-display text-[1.0625rem] font-semibold text-forest-900">
            ...
            <button
              ...
              className="inline-flex items-center gap-2.5 rounded-full bg-forest-800 px-5 py-3 text-[0.875rem] font-semibold text-sand-50 shadow-[var(--shadow-lift)] transition-[background-color,transform] duration-300 ease-[var(--ease-out-soft)] hover:bg-forest-700 motion-safe:hover:-translate-y-0.5"
            >
